...
### 优化 / 重构改动的验收纪律（2026-09-14 立规，源于 slash 菜单不弹出事故）

> 事故样本：`f52487d`「零命中即收起」单测 1313 全绿，真机 `/` 菜单死——单测验逻辑，风险在时序。四条铁律：

1. **验收等级匹配风险类型**：动时序 / UI / IME / 异步数据流的改动，单测只是入场券，**真机手感（人工或 CDP 探针）是通行证**，缺一不交。
2. **「行为等价」必须交证据**：重构类改动用副作用体检——旧版跑快照 → 新版跑快照 → 逐字节 diff（见 `docs/KNOWN-ISSUES.md` #26），不许口头声明等价。
3. **高频 UI 交互有冒烟探针**：按 `/`、`:`、`[[` 弹不弹、Esc 收不收，发版前必跑一遍（探针脚本待建，建好后挂在这里）。
4. **禁手抄重写库函数**：先 profiling 证明热点存在，重写必须附等价性对照测试（样本：`suggestion-guard.ts` 的四个偏离点均需用例锁定）。
...
| parser/serializer / schema（列表容器、mark 定界符、`excludes`） | `fixtures.spec.ts`（重开等价）+ `roundtrip.spec.ts` **Phase E**（mark 嵌套「后开先关」契约）+ `schema-contract.spec.ts`（生产 schema 正向契约锁：容器 / mark 互斥 / attrs）+ `docs/cjk-boundary.md`（CJK 边界）+ `ARCHITECTURE.md §11` 敏感区 #15 / #16                                                        |
...
| **核查功能完善度**   | [docs/FEATURE-MATRIX.md](./docs/FEATURE-MATRIX.md) | [docs/KNOWN-ISSUES.md](./docs/KNOWN-ISSUES.md) → [ARCHITECTURE.md](./ARCHITECTURE.md) §11 |
...
- **IME 候选窗失锚连修多层：「猜层」vs「看层」、「加法守卫」vs「减法根因」**（2026-09-05）：输入法候选窗塌成小方块，先后修三层才接近根治——① 事务层：搜索高亮/焦点模式/代码块高亮在组字期重建装饰改 DOM，各加 `view.composing` 守卫；② 渲染层：P5-02 `content-visibility:auto` 无条件作用于所有块，隐含 containment 干扰 WebView2/TSF 组字光标矩形，收敛为仅大文档启用；③ 布局层：`<ErrorBoundary class=editor-area>` 双根 fragment 致 class 被 Vue 丢弃，`min-width:0` 不生效、编辑区宽度随内容重排、光标矩形抖动，用独立 div 承载 class。**教训**：① 跨层 bug 逐层「猜」会连修多版都不中（字体 bug 猜了四版、IME 猜了三层）；正确做法是第一轮就加运行时诊断（组字期 MutationObserver + 光标矩形采样）「看」真实行为再定位。② 给症状加守卫（在 N 个插件里各塞一个 composing 判断）是加法、会扩散复杂度；能减则减——优先移除/门控惹祸特性（content-visibility），或把守卫收敛到单一中枢，而非每处复制。③ 性能优化上线前的兼容性清单必须包含输入法/无障碍等「系统级交互」，P5-02 清单漏验 IME 直接埋雷。④ 「看似合理的优化/修复」≠ 产品真的变好：正确性修复（丢图/脏标/重载）有回归锁可证；IME 与性能收益在真机确认前一律标注「未验证」，不得写成已提升。**最终判定（2026-09-05 组字期诊断）**：组字期光标矩形从未退化（badSampleCount:0）、DOM 仅组字文本自身 characterData、无 refocus、WebView2 运行时新版 152 → 残差为 **WebView2/TSF 外部间歇缺陷**，编辑器层封顶、不再加守卫，临时诊断 `ime-diag.ts` 已删；content-visibility 门控（仅大文档启用）保留，但理由修正为「把有 caret/TSF 风险的渲染优化限定在真正需要它的大文档」，而非 IME 保护（该成因已被证伪）。
- **出站剪贴板：一个产出覆盖所有目标 = 必然有目标受委屈**（2026-09-12 剪贴板范式改造）：`clipboardTextSerializer` 曾**无条件**把 `text/plain` 覆盖成 Markdown 源码（为让 callout/公式/mermaid 粘到外部 MD 编辑器不丢），于是微信/Word/记事本/搜索框全吃到 `a\*b`、`\#井号` 这类「为 Markdown 目标做的」转义。**教训**：① 对外产出要**按目标分流**——富格式走 HTML（ProseMirror 默认）、纯文本走渲染后的文字、源码走显式入口（`edit.copyAsMarkdown`），别用一个产出服务所有目标；② 「要不要回落」的判定用**白名单 + 退化安全**（`PLAIN_TEXT_SAFE_NODES`，新增节点默认回落）比黑名单稳，漏登记也不会静默丢内容；③ 动这类产出策略前先查**谁还共享这条路径**——本次实测证明文件落盘与剪贴板共用 `escapeInline`，先前"砍转义集"方案（B）会误伤落盘，被 79 份真实语料回放否证（14 份语义漂移），故整条砍掉；④ 剪贴板仍有 text/html 与 text/plain **两条独立管道**，改一条必须确认另一条与 solo→solo 粘贴不受影响（本次靠「各扩展都有 `parseHTML`、粘贴走 HTML」确认）。
- **表格复制粘出 GFM 表格源码：PM slice 开口语义 + 「两条管道只修了一条」**（2026-09-07）：v1.2.43 修「表格选区粘到外部带表格壳」时只拦了 CellSelection 的 text/html（当时有意「text/plain 不动」），用户反馈格内双击选两个字复制，粘到聊天框仍变 `| 剪映 |\n| ---- |`。真根因在 text/plain 管道的 `serializeClipboardSlice`：`doc.copy(slice.content)` 会丢掉 PM slice 的开口标记（openStart/openEnd），选区落在容器内（表格/引用块/列表）时，边界处的 table/row/cell 等容器节点是「部分包含」的开口层，被序列化器当闭合节点整段渲染。修复：`stripOpenLayers` 剥开口层（table 系空 handler 节点随时剥、语义容器按开口计数剥、停在 textblock——顶层 inline 会被 renderContent 输出为空）。**教训**：① 剪贴板有 text/html 与 text/plain **两条独立管道**（同「CSP 与 CORS 是两道独立闸门」），修了一条必须验证另一条对同一场景的行为，v1.2.43 只测了富文本目标（Word/WPS）；② PM 的 `slice.content` 语义坑：开口边界节点原样在 fragment 里、`doc.copy` 不校验 schema 直接构造，凡「取选区再序列化」的代码都要先问选区是否落在深层容器内；③ 测试 schema（`test-utils.ts`）的 table 节点没定义 colspan/rowspan attrs（**2026-09-14 已补**：现已定义 `colspan/rowspan/colwidth`），`prosemirror-tables` 的 `TableMap.findWidth` 对 undefined 求和得 NaN，CellSelection 在单测里无法构造——测 CellSelection 相关序列化要用 `Slice(Fragment.from(rows), 1, 1)` 同构构造。
