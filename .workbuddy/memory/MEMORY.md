...
### 文档权重约定（2026-07-21 确立，重要）
- **开发纲领 / 工作纪律 / 产品精神 = 宪法级 + 愿景级**，真身须留在根级 `ARCHITECTURE.md` / `AGENTS.md` / `README.md`，**不可降级到 `docs/` 补遗区**（`docs/` 在 solo 体系是「专题/问题导向」辅助区，降进去=稀释接手者注意力权重）。
- 三份原则草稿（产品开发总原则 / 工作原则和纪律 / 产品精神）是根级正式文档的**详细母本**：其核心已吸收进根级；独有细节（决策阶梯、安全底线清单、不擅自下载软件铁律、信息同步纪律、产品精神 6 关键词）已**合并回根级** ARCHITECTURE.md / AGENTS.md。
- **原文件保留在 `docs/` 作跨项目方法论模板**（用户决定：不删、可复用于其他项目开发）。属「跨项目复用资产」非 solo 专属——若日后集中管理，更优位置是仓库外的 `~/dev-standards` 独立目录。
