...
> **写作基准**：一切以**实际代码行为**为准，不依据注释或历史文档。版本对应 `package.json` v1.2.7。
...
| 测试 | Vitest + happy-dom | 4.x | 27 个 spec 文件（977 tests） |
| 包管理 | bun | 1.3.14 | |
...
│         (11 个，按       ┌─ file.ts       App ▸ Editor           │
│          关注点拆分)     └─ settings.ts   ▸ Settings(22)         │
│                                                ▸ Layout 等       │
└──────────────────────┬──────────────────────────────────────────┘
...
│   │   ├── useExportActions.ts   #     导出：HTML/PDF/微信
...
└── package.json                  #   name=marklight (历史残留), version=1.2.7
```
...
| `resolve_document_image_path` | document.rs | 相对路径 → 绝对路径 |
| `authorize_image_asset` | document.rs | 把图片加入 asset 协议作用域（**带安全校验**） |
| `fetch_remote_image` | image.rs | 下载远程图片（≤10MB）→ base64 data URL |
| `print_document` | window.rs | 调系统打印（PDF 导出走此路） |
| `reveal_in_finder` | window.rs | 在资源管理器中定位 |
| `set_window_background_color` | window.rs | macOS 窗口背景（NSColor） |
| `register_shell_new` / `unregister_shell_new` | desktop.rs | Windows 右键"新建 Markdown"注册表 |
| `consume_startup_open_request` | lib.rs | 取出启动期累积的开打请求 |
...
| `useMenuShortcutsSync` | 快捷键同步 | 自定义快捷键变更 → Rust 重建菜单 |
| `useExportActions` | 导出动作 | HTML/PDF/微信 |
...
13 个设置项（activeThemeId / customThemes / fontSize / fontFamily / autoSave / autoSaveInterval / spellCheck / titlebarAutoHide / lineHeight / wechatTheme / customShortcuts / customEditorCSS / configVersion）。

- **持久化**：tauri-plugin-store（经 `services/tauri/store.ts`）。焦点模式额外用 `localStorage`（key `marklight-focus-mode`，历史残留命名）。
- **迁移**：`configVersion`（当前 6）。加载时若存储的版本 ≠ 当前版本 → 规范化后**回写**，一次性升级。
...
**不走** ProseMirror 序列化，而是建一棵中间表示树再渲染：

```
ProseMirror Doc
   → buildExportTree() ── ExportDocument (model.ts 定义的 IR)
                          ├─ HTML 渲染器: renderEditorDocToHtmlDocument (用编辑器主题色)
                          └─ Wechat 渲染器: renderEditorDocToWechatFragment (用微信主题色)
```
...
| 导出 HTML | `export.html` | — |
| 导出 PDF | `export.pdf` | Mod+Shift+P |
| 微信导出 | `export.wechat` | Mod+E |
| 查找 | `edit.find` | Mod+F |
