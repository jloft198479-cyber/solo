...
async function renderBlock(block: ExportBlock, target: 'html' | 'wechat'): Promise<string> {
  switch (block.kind) {
    case 'paragraph':
      return `<p class="ml-export-paragraph">${await renderInlines(block.inlines, target)}</p>`;
    case 'heading':
      return `<h${block.level} class="ml-export-heading ml-export-heading-${block.level}">${await renderInlines(block.inlines, target)}</h${block.level}>`;
    case 'blockquote':
    ...
    case 'table': {
      const rows = await Promise.all(
        block.rows.map(async (row) => {
          const cells = await Promise.all(row.cells.map(renderTableCell));
    ...
    case 'mathBlock':
      return await renderMathBlock(block.latex);
    case 'mermaidBlock':
...
async function renderListItemBody(
  ...
  if (blocks.length === 1 && (blocks[0].kind === 'paragraph' || blocks[0].kind === 'heading')) {
    ...
    if (block.kind === 'paragraph') {
      return `<span class="ml-export-list-item-inline">${await renderInlines(block.inlines, target)}</span>`;
    }
    return `<span class="ml-export-list-item-inline">${await renderInlines(block.inlines, target)}</span>`;
  }
...
async function renderTableCell(cell: {
  kind: 'tableHeader' | 'tableCell';
  blocks: ExportBlock[];
}): Promise<string> {
  const tag = cell.kind === 'tableHeader' ? 'th' : 'td';
  const body = (await Promise.all(cell.blocks.map((block) => renderBlockSync(block)))).join('');
  return `<${tag} class="ml-export-table-cell">${body}</${tag}>`;
}

async function renderBlockSync(block: ExportBlock): Promise<string> {
  switch (block.kind) {
    case 'paragraph':
      return `<p class="ml-export-paragraph">${await renderInlines(block.inlines, 'html')}</p>`;
    case 'heading':
      return `<p class="ml-export-table-heading">${await renderInlines(block.inlines, 'html')}</p>`;
    case 'blockquote': {
      const inner = (await Promise.all(block.blocks.map(renderBlockSync))).join('');
      return `<blockquote class="ml-export-blockquote">${inner}</blockquote>`;
    }
    case 'bulletList': {
      const items = await Promise.all(
        block.items.map(async (item) => {
          const inner = (await Promise.all(item.blocks.map(renderBlockSync))).join('');
    ...
    case 'orderedList': {
      ...
      return `<ol class="ml-export-list ml-export-list-ordered">${items.join('')}</ol>`;
    }
    case 'taskList': {
      const items = await Promise.all(
        block.items.map(async (item) => {
    ...
    case 'callout': {
      const inner = (await Promise.all(block.blocks.map(renderBlockSync))).join('');
      return `<aside class="ml-export-callout" data-callout-type="${escapeAttribute(block.calloutType)}"><div class="ml-export-callout-title">${escapeHtml(block.title || capitalize(block.calloutType))}</div><div class="ml-export-callout-body">${inner}</div></aside>`;
    }
    case 'horizontalRule':
      return '<hr class="ml-export-hr" />';
  }
...
async function renderInlines(inlines: ExportInline[], target: 'html' | 'wechat'): Promise<string> {
  const results = await Promise.all(inlines.map((inline) => renderInline(inline, target)));
  return results.join('');
}

async function renderInline(inline: ExportInline, target: 'html' | 'wechat'): Promise<string> {
  let content: string;
    ...
    case 'mathInline':
      content =
        target === 'wechat'
          ...
          : await renderInlineMath(inline.latex);
      return wrapMarks(content, inline.marks, renderMark);
    case 'wikilink': {
...
async function renderInlineMath(latex: string): Promise<string> {
  const katex = (await getKatex()).default;
...
async function renderMathBlock(latex: string): Promise<string> {
  const katex = (await getKatex()).default;
