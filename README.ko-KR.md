...
- **형식 충실도** — 완전한 Markdown 왕복 테스트 + CommonMark spec 안정성 검증(618 통과 / 34 설계 제약). Markdown 붙여넣기 자동 변환, Ctrl+C는 정리된 일반 텍스트를 복사합니다(수식·다이어그램·위키링크 등이 포함된 선택 시에만 Markdown 소스). 소스가 필요하면 **Copy as Markdown**(Mod+Shift+M)을 사용하세요.
