...
- **フォーマット忠実性** — 完全な Markdown ラウンドトリップテスト + CommonMark spec 安定性検証（テスト数は `bun run test` を参照）。Markdown貼り付け自動変換、Ctrl+C は整形済みのプレーンテキストをコピー（数式・図表・ウィキリンク等を含む選択時のみ Markdown ソース）。ソースが必要なときは **Copy as Markdown**（Mod+Shift+M）。
