...
- **省メモリ** — フォーカス喪失時に WebView2 MemoryUsageTargetLevel 低減、エディタ遅延初期化、インストーラ約5MB
- **HTML エクスポート** — テーマに従ったエクスポート、見たままを出力
- **フォーマット忠実性** — 977 のラウンドトリップテスト + 652 の CommonMark spec 安定性検証（618 合格 / 34 設計制約）。Markdown貼り付け自動変換、Ctrl+Cでソースもクリップボードへ
