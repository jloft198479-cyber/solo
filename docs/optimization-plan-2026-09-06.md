...
> 状态：**P0-P2 主批已执行**（见第三节路线图勾销）；剩余：P2 次级（B5/B10）、B3 独立小批、P3。每完成一项：[`KNOWN-ISSUES.md`](./KNOWN-ISSUES.md) 对应条目移 §一 已修复 + 本文勾销进度。
...
| **P2** | 格式兼容 | ✅ 主批完成（2026-09-06）：B1、B2、B7、B8、B6、B4（先红后绿，逐项 roundtrip 测试）+ 顺手修复 CommonMark spec Ex20/603（destination 反斜杠转义，652 条规范用例 0 失败）；B5、B10（美观/边缘）待做；B3 Word 列表重建工作量最大，独立小批 | bun run test（1241 全过）+ vue-tsc + build 三步全过 ✅ |
