...
| 9 | 拖拽落点**跟手性**待真机确认（互链一手动验证项；坐标系已由源码确认） | 落点位置按「物理像素 ÷ devicePixelRatio → `elementFromPoint` 命中编辑器 → `view.posAtCoords` 反查文档位置」实现。**坐标系已查证**：wry 0.55.1 `webview2/drag_drop.rs` 用 `ScreenToClient(container HWND)`，而 container 覆盖窗口客户区（`create_container_hwnd` 非 child 分支取 `parent_bounds` 且落点 (0,0)）⇒ position = **viewport 物理像素、不含标题栏**，与 `elementFromPoint`/`posAtCoords` 的 CSS 逻辑像素除以 dpr 后同源。**仍需真机确认**：多屏混合 DPI、显示缩放下的跟手性。失败模式非破坏：误判为正文外→回落「打开」；落点在代码块/取不到→回落 fallback 或退回光标。**验证清单**：① 拖同目录 `.md` 到正文中部 → 应插在**松手处**；② 拖到正文外 → 应「打开」；③ 拖图片 → 仍插图；④ 光标在代码块内拖 `.md` → 应「打开」；⑤ 拖当前文档自身 → 应「打开」（不成自链接） | [`wikilink-drop.ts`](../src/components/Editor/tiptap/extensions/wikilink-drop.ts) / [`MarkdownEditor.vue`](../src/components/Editor/MarkdownEditor.vue) |
