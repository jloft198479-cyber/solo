...
| 16 | 表格选区复制粘到外部自带完整 `<table>` + 像素死宽度 | 跨单元格选区被 prosemirror-tables 升级为 `CellSelection`，其 `content()` 返回整表/表格行，PM `wrapMap` 补 `<table><tbody>` 壳，`resizable:true` 带固定像素 `<colgroup>`；Word/WPS/微信等富文本目标优先取 text/html | copy 事件拦截：仅 CellSelection 时把 text/html 压平为逐行 `<p>`（单元格间 `\t`，粘 Excel/WPS 自动分列）；text/plain 管道不动；普通选区富格式保真不动 | [`MarkdownEditor.vue`](../src/components/Editor/MarkdownEditor.vue) `onEditorCopy` |
