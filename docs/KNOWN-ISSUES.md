...
| 9 | 拖拽落点坐标**未真机验证**（互链一手动验证项） | 拖入成链的落点位置按「物理像素 ÷ devicePixelRatio → `elementFromPoint` 命中编辑器 → `view.posAtCoords` 反查文档位置」实现，**标题栏偏移 / 显示缩放下的跟手性未在真机验证**。失败模式非破坏：误判为正文外→回落「打开」；落点在代码块/取不到→回落 fallback 或退回光标。**验证清单**：① 拖同目录 `.md` 到正文中部 → 应插在**松手处**；② 拖到正文外 → 应「打开」；③ 拖图片 → 仍插图；④ 光标在代码块内拖 `.md` → 应「打开」；⑤ 拖当前文档自身 → 应「打开」（不成自链接）。不稳则按方案回落修饰键兜底 | [`wikilink-drop.ts`](../src/components/Editor/tiptap/extensions/wikilink-drop.ts) / [`MarkdownEditor.vue`](../src/components/Editor/MarkdownEditor.vue) |
