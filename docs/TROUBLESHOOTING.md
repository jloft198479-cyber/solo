...
audience: user
status: active
tags: [故障排查, 用户, 注册表, 自动保存]
summary: 用户侧运行时问题排查（症状→修法），编译问题见 BUILD_GUIDE
updates: [src-tauri/src/commands/document.rs, src-tauri/nsis-hooks.nsh, BUILD_GUIDE.md]
---
...
**原因**：solo 使用多进程架构（v1.2.5+），每双击一个 `.md` 文件启动一个独立进程，关闭最后一个窗口时进程默认不退出。
**解决**：从菜单选择「退出」→ 会向所有窗口发送关闭请求，逐窗口确认未保存内容后正常退出（任一窗口点「取消」则中止退出）。如果菜单点了没反应：
- 任务管理器中有没有残留 `solo.exe` 进程，手动结束即可（不影响其他进程打开的窗口）

> v1.2.5 起不再使用单实例；一个窗口卡死或被强制结束进程，**不牵连**其他窗口。

### 7. 编辑器内容丢失/未保存

solo 有自动保存机制（文档修改后 2s 自动保存），且**原地保存到原文件**（原子写：先写隐藏 `.tmp` 再改名替换），非临时副本。如果遇到内容丢失：
1. 先看原文件本身——2s 自动保存就是写原文件，文件未损坏时通常无需恢复。
2. 若原文件损坏/被覆盖，检查同目录下是否有隐藏残留 `.tmp`（命名 `.{原文件名}.{数字}.tmp`），改回原名即为最近一次写入（可能是最后状态）。
3. 崩溃残留的 `.tmp` 会在下次打开该文档时自动清理（超过 1 小时的视为 stale，见 `src-tauri/src/commands/document.rs::cleanup_stale_tmp_files`），无需手动处理。

> 设置（主题/字体等偏好）存于 `%APPDATA%\com.solomarkdown\settings.json`（tauri-plugin-store，key 为 `settings`），与文档恢复无关；solo 无「最近文档列表」持久化（单文件编辑器，多窗口各开各的）。
