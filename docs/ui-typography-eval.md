...
**P2 · 行长微收：720px → 680px → 自适应 680–960px**

- 现状：720px @16px ≈ 42 个中文字/行
- 最佳实践：中文舒适行长 30–40 字（英文 45–75 字符的对应换算）
- 建议：容器收到 680px（≈39 字/行），更接近"书卷气"的阅读节奏；代价是每行少约 2 字
- **2026-08-22 更新**：680px 钉死导致全屏时内容呈窄条（大屏两侧大量留白），改为自适应——`--mk-content-max-width: clamp(680px, 52vw, 960px)`：窗口 ≤1307px 保持 680px，1307–1846px 随窗口渐进加宽，≥1846px 封顶 960px（≈60 字/行）。打印模式不受影响（`max-width: 100% !important` 覆盖）。
...
1. **「文字变浅」：按 2.3 方案落地**（HTML span + 自定义 Mark）。spike 实证（见 2.6）确认需补 parser 识别 + serializer 规则两层、而非"零插件零成本"，据此实现 + 单测（roundtrip / 裸 `</span>` 防误吞 / 嵌套 mark）**已在 2026-08-21 完成**，测试、类型、构建全通过。详见 §二 追加记录。
