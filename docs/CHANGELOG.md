...
> 2026-09-15 系统性评估（[`REVIEW-2026-09-15-体验与代码评估.md`](./REVIEW-2026-09-15-体验与代码评估.md)）修复批次。版本号与发布动作待发版时执行。

### Fixed
- **列表项内嵌块「脱出列表」+ 二次不收敛（#27）**：列表项的续行未缩进到项内容列，项内代码块 / 引用 / 表格 / 图片 / 分隔线被写到第 0 列，重开变成三个平级块，且每保存一次再变一次。序列化器引入 `lineIndent` 内容列缩进累积，块序列化器不再各自关心缩进。
- **嵌套列表紧凑度被撑开（#27）**：子列表缩进取 marker 内容列（2/3/4）而非固定 3 空格，父项后不再多插空行（不再读成 loose）。
- **文件模式转义「按语境」重构，消灭「首次保存即改文件」（#28）**：旧版无条件转义 `` ` [ ] ( ) * ~ ^ = | $ < > { } ``，打开再保存就会把 `List<String>` 写成 `List\<String\>`。改为按「会不会改变解析结果」判定（恒需 / 成对才需 / 视位置才需 / 无需）。顺带修正段首 `1. 文字` 存盘后被读回有序列表、`- >文字` 变嵌套引用、行首整行 `=` 被读成 setext 标题（CommonMark Ex 93 不收敛）。
- **表格列对齐 `:--` / `:-:` / `--:` 读写双向丢失（#29）**：parser 从单元格 token 的 `text-align` 提取对齐写进 `align`，serializer 分隔行按列输出对齐符（表头未设时回落数据行）。
- **HTML 实体「层层剥皮」（#29）**：对实体形态的 `&`（`&amp;` / `&#65;` / `&#x42;`）转义，防止 `&amp;lt;` 每保存一次剥一层、两轮不收敛。
- **code-block 打开时同步高亮全部代码块（#30）**：几十个未标注语言的块要逐个跑 17 种 tokenizer，同步跑完 ≈330ms（Node）/ 0.65~1s（WebView2），全砸在首帧。**同批**：空闲补算在组字（IME）期间整体推迟——合并装饰会改动组字中的 `<code>` DOM，正是候选窗失锚的成因（见 `composition-freeze` 铁律）。

### Performance
- **序列化 O(n²) → O(n)（#18）**：根因是 `output +=` 与随后的 `endsWith` / 下标读取（读即摊平整串）。改为分片数组累积，**443,259 字文档 20,487.82 → 127.95ms（160.1×）**，三档输出逐字节一致。
- **代码块高亮判据按块规模判定 + 打开时分批（#30）**：自动语言检测成本只跟代码块自身大小走，加 3000 字符/块上限；打开时同步只高亮前 8 块，其余交既有空闲调度器每批 4 块补算（进度用节点引用 + `WeakSet` 判定，无重复计算）。
